...
class ImageCodeDataset(Dataset):
    ...
    def __init__(self, 
        ...
        if pred_path is not None:
            self.prediction = np.load(pred_path, allow_pickle=True).item()
        else:
        ...
        if self.mask_tree:
            ...
            self.code_cond_lens = self.hc["len"] if "len" in self.hc.keys() else None
    ...
    def __getitem__(self, index: int) -> Dict:
        ...
        if self.has_rect:
            ...
            if self.is_short:
                ...
                if self.normalize_rect:
                    rects = np.empty((1, 4), dtype=np.float32)
                    rects[0] = rect
        ...
        if self.mask_tree:
            if self.cvs is not None:
                ...
                if self.code_cond_lens is not None:
                    item["code_cond_len"] = self.code_cond_lens[code_idx]
            else:
                assert False
                # code = item["code"]
                ...
                #     item["code_cond"] = cond_masked
                #     item["code_len"] = i + 1
                #     item["code_cond_len"] = self.code_cond_lens[]

        #

        if self.prediction is not None:
            item["pred"] = self.prediction["predicts"][code_idx]
