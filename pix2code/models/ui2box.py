...
from .mlp import MLP
from .vit2box import compute_kl_loss, compute_w2_loss
...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.shape
        not_mask = torch.zeros((b, h, w), dtype=torch.bool, device=x.device)
...
def build_position_encoding(position_embedding = "sine", 
    ...
    if position_embedding in ('v2', 'sine'):
        ...
        pos_embedding = PositionEmbeddingSine(N_steps, normalize=True)
    elif position_embedding in ('v3', 'learned'):
        pos_embedding = PositionEmbeddingLearned(N_steps)
    else:
...
class Transformer(nn.Module):
    ...
    def forward(self, src, tgt, tgt_mask, query_embed, pos_embed):
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt = self.norm1(tgt)
        # print(self.with_pos_embed(tgt, query_pos).shape)
        # print(self.with_pos_embed(memory, pos).shape)
...
class Ui2Box(nn.Module):
    ...
    def forward(self, batch):
        ...
        r = {
            "loss": loss,
            "scores": preds_box_convert.detach(), 
            "targets": truth_box_convert.detach(),
            "pb": preds_box.detach(),
            "tb": truth_box.detach()
        }
