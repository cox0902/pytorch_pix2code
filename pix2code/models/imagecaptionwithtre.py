...
class Rnn(nn.Module):
    ...
    def __init__(self, 
        ...
        self.rnn_step = nn.LSTMCell(embed_dim + encoder_dim + decoder_dim, decoder_dim, bias=True)
    ...
    def forward(self, x, y, z, hidden):
        x_emb = self.embedding(x)
        y_hat, _ = self.attention(y, hidden[0])
        return self.rnn_step(torch.cat([x_emb[None], y_hat, z], dim=1), hidden)
...
class TreeNode:
    ...
    @staticmethod
    def _train(n: "TreeNode", prds: List, tgts: List, fc, rnn: Rnn, y):
        # print("=" * 100)
        if len(n.left.children) > 0:
            left_iv = torch.tensor(1).to(n.iv.device)
        else:
            left_iv = n.left.iv
    ...
    @staticmethod
    def _assign_init_hidden_state(n: "TreeNode", hidden):
        if n.iv == 3:
            n.hidden = hidden
    
    def train(self, fc, rnn: Rnn, y):

        prds, tgts = [], []

        hidden = rnn.init_hidden_state(y)
        # self.hidden_h = rnn_h.init_hidden_state()
        # self.hidden_v = rnn_v.forward(self.iv, rnn_v.init_hidden_state())
        # self.hidden_v = rnn.init_hidden_state(y)
        self.hidden = hidden
        ...
        self.preorder_walk_children(partial(TreeNode._assign_init_hidden_state, hidden=hidden))
...
class DecoderWithAttention(nn.Module):
    ...
    def __init__(self, max_len, attention_dim, embed_dim, decoder_dim, vocab_size, 
        ...
        self.rnn = Rnn(self.embedding, self.attention, embed_dim, encoder_dim, decoder_dim)
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths, return_alphas = False):
        ...
        for bi in range(batch_size):
            ...
            prd, tgt = tree.train(self.fc, self.rnn, encoder_out[bi][None])
...
class ImageCaptionWithTre(nn.Module):
    """
